...
processed_path = '/study3/midus3/processed_data/MIDUS3_Imaging'
...
for file in txt_files:
    ...
    tsv_path = processed_path + "/sub-%s/func/"%(name) # name extracts first ###

    if os.path.isfile(processed_path + "/Temporary/Big/sub-%s_task-ER_events.tsv"%(name)) == True:
        print "----------No E-prime Conversion for", name, "Needed----------"

    elif os.path.isfile(processed_path + "/Temporary/Big/sub-%s_task-ER_events.tsv"%(name)) == False:
        print "----------Converting----------"
        # Don't do anything when .tsv is already there
        if os.path.isfile(processed_path + "/sub-%s/func/sub-%s_task-ER_events.tsv"%(name, name)) == True:
            print ".tsv file(s) already exists for " + name
        
        # Don't do anything when .tsv with any version number is already there
        elif os.path.isfile(processed_path + "/sub-%s/func/sub-%s_task-ER_events_1.tsv"%(name, name)) == True and os.path.isfile(processed_path + "/sub-%s/func/sub-%s_task-ER_events_2.tsv"%(name, name)) == True:
            print ".tsv file(s) already exists for " + name

        # Convert when .tsv is not there
        elif os.path.isfile(processed_path + "/sub-%s/func/sub-%s_task-ER_events.tsv"%(name, name)) == False:
            if "009" in name: # Currently hardcoded for 009 --> more global approach needed using file counter
                ...
                os.chdir(tsv_path)
                tsv = processed_path + "/Temporary/Big/sub-%s_task-ER_events_%s.tsv"%(name, versionNumber)
                os.system("eprime2tabfile " + file + " > " + tsv)
                versionNumber += 1
            else: 
                print "----------Creating .tsv file for " + name + "----------"
                os.chdir(tsv_path)
                tsv = processed_path + "/Temporary/Big/sub-%s_task-ER_events.tsv"%(name)
                os.system("eprime2tabfile " + file + " > " + tsv)
