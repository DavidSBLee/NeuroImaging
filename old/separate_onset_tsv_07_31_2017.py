...
outfile = '/study3/midus3/processed_data/MIDUS3_Imaging/OnsetTest/'
...
for tsv in tsvPath:
	...
	with open(tsv, 'rb') as tsvFile, open('/study3/midus3/processed_data/MIDUS3_Imaging/OnsetTest/Temporary/sub-' + subNum + '_computed-01.tsv', 'wb') as newTsv:
		...
		for rows in rowReader:
			...
			if "1" in blockNum:
				rowWriter.writerow({'Onset_IAPS': IAPSOnsetTime/1000,
					'Duration_IAPS':IAPSDuration,
					'Onset_Face': faceOnsettime/1000,
					'Duration_Face': faceDuration,
					'Response_Time_Face': responseTime,
					'Blocks':rows['Blocks']})
					

	with open(tsv, 'rb') as tsvFile, open('/study3/midus3/processed_data/MIDUS3_Imaging/OnsetTest/Temporary/' + subNum + '-temporary-01.tsv', 'wb') as newTsv:
	...
	df4.loc[(df4['Response_Time_Face'] < 0 ) | (df4['Response_Time_Face'].isnull()), 'Response_Time_Face'] = "NaN"
	df4.to_csv(outfile + subNum + "_task-ER_run-01_events.tsv", sep='\t', index=None) # index = None --> removes first column of the index which are created by to_csv Pandas dataframe
	
	print "----------Onset file(s) for " + subNum + " created----------"

	### Creating Onset for block 2 ###

	with open(tsv, 'rb') as tsvFile, open('/study3/midus3/processed_data/MIDUS3_Imaging/OnsetTest/Temporary/sub-' + subNum + '_computed-02.tsv', 'wb') as newTsv:
		fields = ('Onset_IAPS',
		'Duration_IAPS',
		'Onset_Face',
		'Duration_Face',
	...
	with open(tsv, 'rb') as tsvFile, open('/study3/midus3/processed_data/MIDUS3_Imaging/OnsetTest/Temporary/' + subNum + '-temporary-02.tsv', 'wb') as newTsv:
	...
	# Use loc and isnull to replace negative responset times to "NaN"
	df5.loc[(df5['Response_Time_Face'] < 0 ) | (df5['Response_Time_Face'].isnull()), 'Response_Time_Face'] = "NaN"
	df5.to_csv(outfile + subNum + "_task-ER_run-02_events.tsv", sep='\t', index=None)

	print "----------Onset file(s) for " + subNum + " created----------"

	with open(tsv, 'rb') as tsvFile, open('/study3/midus3/processed_data/MIDUS3_Imaging/OnsetTest/Temporary/sub-' + subNum + 'computed-03.tsv', 'wb') as newTsv:
		fields = ('Onset_IAPS',
		'Duration_IAPS',
		'Onset_Face',
		'Duration_Face',
	...
	with open(tsv, 'rb') as tsvFile, open('/study3/midus3/processed_data/MIDUS3_Imaging/OnsetTest/Temporary/' + subNum + '-temporary-03.tsv', 'wb') as newTsv:
		writer = csv.writer(newTsv, delimiter='\t', lineterminator='\n') #lineterminator is paramter in csv.writer. it is default set to '\r\n' causing double spacing
		reader = csv.reader(tsvFile, delimiter='\t')
	...
	df1 = pd.read_csv('/study3/midus3/processed_data/MIDUS3_Imaging/OnsetTest/Temporary/sub-' + subNum + 'computed-03.tsv', sep='\t')
	...
	# Use loc and isnull to replace negative responset times to "NaN"
	df5.loc[(df5['Response_Time_Face'] < 0 ) | (df5['Response_Time_Face'].isnull()), 'Response_Time_Face'] = "NaN"
	df5.to_csv(outfile + subNum + "_task-ER_run-03_events.tsv", sep='\t', index=None)
