	...
	if f not in niftis:
		...
		for d in direc:
			...
			if "task-ER_run-1_bold" in scanName:
				...
				os.system("dcm2niix -f " + niftiFolderName + "_" + scanName + " -o /study/midus3/processed_data/my_dataset/" + "sub-" + subNum + "/func/Orientation_To_Fix/" + " -z y /study/midus3/processed_data/dicomdir/" + dicomFolderName + "/" + scanName)
				print  "----------" + scanName + " Conversion Complete----------"
			...
			if "dwi" in scanName:
				...
				os.chdir("dwi")
				os.makedirs("Orientation_To_Fix")
				os.system("dcm2niix -f " + niftiFolderName + "_" + scanName + " -o /study/midus3/processed_data/my_dataset/" + "sub-" + subNum + "/dwi/Orientation_To_Fix/" + " -z y /study/midus3/processed_data/dicomdir/" + dicomFolderName + "/" + scanName)
				print  "----------" + scanName + " Conversion Complete----------"
...
#MK: If we're going to build in a check (i.e., to make sure we aren't           duplicating scans that have already been fixed), we should add it here.         That way, we can visually look at which subjects are being processed and        make sure that it makes sense (e.g., if we see sub-001, we know                 something is wrong!). Or, maybe we shouldn't do this for the whole thing, but by each scan, since some Pp will not have all scans.
	...
	if os.path.isdir("%s/func/Orientation_To_Fix/"%(sub)) == True:
		...
		os.system("fslreorient2std %s/func/Orientation_To_Fix/sub-"%(sub) + subNum + "_task-ER_run-1_bold %s/func/sub-"%(sub) + subNum + "_task-ER_run-1_bold")
		os.system("fslreorient2std %s/func/Orientation_To_Fix/sub-"%(sub) + subNum + "_task-ER_run-2_bold %s/func/sub-"%(sub) + subNum + "_task-ER_run-2_bold")
		os.system("fslreorient2std %s/func/Orientation_To_Fix/sub-"%(sub) + subNum + "_task-ER_run-3_bold %s/func/sub-"%(sub) + subNum + "_task-ER_run-3_bold")
		os.system("fslreorient2std %s/func/Orientation_To_Fix/sub-"%(sub) + subNum + "_task-rest_bold %s/func/sub-"%(sub) + subNum + "_task-rest_bold")
	...
	#if os.path.isfile("%s/func/sub-"%(sub) + subNum + "_task-ER_run-1_bold" == True and os.path.isfile("%s/func/sub-"%(sub) + subNum + "_task-ER_run-2_bold" == True and os.path.isfile("%s/func/sub-"%(sub) + subNum + "_task-ER_run-3_bold" == True and os.path.isfile("%s/func/sub-"%(sub) + subNum + "_task-rest_bold" == True:
...
#MK-Edits to include dwi/dti. Currently placing files in 'Orientation_Fixed' folder (as with func scans), but may want to just replace old files or place 'fixed' files in same directory w/ new name.
	if os.path.isdir("%s/dwi/Orientation_To_Fix"%(sub)) == True: #and os.path.isfile("%s/dwi/Orientation_Fixed/sub-"%(sub) + subNum + "_dwi.nii.gz") == False:
		...
		os.system("fslreorient2std %s/dwi/Orientation_To_Fix/sub-"%(sub) + subNum + "_dwi %s/dwi/sub-"%(sub) + subNum + "_dwi")
	if os.path.isfile("%s/dwi/sub-"%(sub) + subNum + "_dwi") == True:
