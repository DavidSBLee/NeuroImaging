...
data <- read.csv("/home/slee/Desktop/ref_anon.xlsx", header=TRUE)
...
### corrleation
# Linear Regression
lm.model <- lm(ssd_rest_percent ~ rest_percent, data = data)
summary(lm.model)

# Bivarite Association (Two quantitative variables)
library(ggplot2)
theme_update(plot.title = element_text(hjust = 0.5))
ggplot(data, aes(x= SSD, y= FD)) +
  geom_point(shape=19) + 
  xlab("SSD(mm)") + 
  ylab("FD(mm)") + geom_smooth(method=lm, se=FALSE) +
  ggtitle("Time Series of SSD & FD in subject 30725", subtitle=NULL)

cor.test(data$SSD, data$FD)
...
boxplot(data$RA5NEPC3, horizontal = TRUE, outline=FALSE)
boxplot.stats(data$RA5CNE)
hist(data$RA5NEPC3)
...
low_neg <- data[which(data$RA5CNE <= 0.4),]
...
old <- data.combined[which(data.combined$RA1PRAGE >= 49),]
old_data <- data.frame(ageGroup = rep("older", nrow(old)), old[,])
young <- data.combined[which(data.combined$RA1PRAGE < 49),]
young_data <- data.frame(ageGroup = rep("younger", nrow(young)), young[,])

# combine new data sets
data.combined.again <- rbind(old_data, young_data)
...
boxplot(RA5NEPC3 ~ corr_neg, data=data.combined)
boxplot(RA1PRAGE ~ corr_neg, data=data.combined, outline=FALSE)

# Independant 2-group t-test
t.test(RA5NEPC3 ~ corr_neg, data=data.combined)
...
anova1 <- aov(RA5NEPC3 ~ corr_neg, data = data.combined)
summary(anova1)
...
plane <- lm(data$RA5PHC ~ data$RA5PHM + data$RA5PB)
s3d$plane3d(plane)
...
reg1 <- lm(age ~ cb_total + free_recall_total,
           data = data.combined)
reg1
summary(reg1)
